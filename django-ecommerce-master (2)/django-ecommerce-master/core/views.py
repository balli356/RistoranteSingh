...
""" questo home page controlla se un clineta fa primo ingresso  """
...
def ProfileView(request):
    ...
    if (request.user.is_staff):
        form = Booking.objects.filter(date_start__month=x)
        form.order_by('date_start','+time')
    else:
        form=Booking.objects.filter(user_id=user,date_start__month=x)
...
def booking(request):
    if request.method == 'POST' and request.user.is_authenticated:
        ...
        if form.is_valid() and sum(object_list.values_list('chair', flat=True)) < 10:
            ...
            messages.error(request, 'Salvataggio non ha avuto successo scgliere un altro giorno!')
...
def add_review(request,slug):
    if request.method == 'POST' and request.user.is_authenticated:
        ...
        if form.is_valid():
            ...
            item.rank.add(review)
            item.save()
            messages.success(request, 'Aggiunto al Review!')
            print("update review")
            return redirect('core:home')
