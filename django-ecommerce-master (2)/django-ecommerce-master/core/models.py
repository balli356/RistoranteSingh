...
class Item(models.Model):
    ...
    rank = models.ManyToManyField(Reviews, null=True)
    ...
    def get_review(self):
        totals= []
        for rankk in self.rank.filter(item__slug__exact=self.slug   ):
            totals.append("User "+(rankk.user.first_name)+" ha dato la sua review ( "+(rankk.description)+" ) con il voto = "+str(rankk.rank))
